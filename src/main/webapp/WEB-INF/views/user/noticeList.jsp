...
<head>
    ...
    <script>
        ...
        function search_article(){
            ...
            console.log(search);
            location.href="/board/notice?pageNum=0&keyword="+search;
        }
...
<body>
    <c:choose>
        ...
        <c:otherwise>
            <jsp:include page="side/side.jsp" flush="true" />
        </c:otherwise>
    </c:choose>
    <div class="wrap">
        <div class="table">
            ...
            <div class="page">
                ...
                <a class="s_f" href="/board/notice?pageNum=0&keyword=${keyword}">
                    <span class="material-icons-outlined sf">first_page</span>
                </a>
                <c:if test="${pageNum > 0}">
                    <a class="pageNum" href="/board/notice?pageNum=${pageNum-1}&keyword=${keyword}">
                        ...
                        <c:when test="${keyword == null}">
                            <c:choose>
                                <c:when test="${pageNum == i}">
                                    <span class="pageNum"><a class="selected" href="/board/notice?pageNum=${i}">${i + 1}</a></span>
                                </c:when>
                                <c:otherwise>
                                    <span class="pageNum"><a href="/board/notice?pageNum=${i}">${i + 1}</a></span>
                                </c:otherwise>
                            </c:choose>
                        </c:when>
                        <c:when test="${keyword != null}">
                            <c:choose>
                                <c:when test="${pageNum == i}">
                                    <span class="pageNum"><a class="selected" href="/board/notice?pageNum=${i}&keyword=${keyword}">${i + 1}</a></span>
                                </c:when>
                                <c:otherwise>
                                    <span class="pageNum"><a href="/board/notice?pageNum=${i}&keyword=${keyword}">${i + 1}</a></span>
                                </c:otherwise>
                            </c:choose>
                        </c:when>
                    </c:choose>
                ...
                <a class="s_f" href="/board/notice?pageNum=${pageCnt}&keyword=${keyword}">
