...
<head>
    ...
    <script>
        ...
        function search_article(){
            ...
            console.log(search);
            location.href="/adm/notice/list?pageNum=0&keyword="+search;
        }
        ...
        function detail_board(id,e){
             location.href="/adm/notice/detail/"+id;
        }

        function fn_delete(id){
            if(confirm('공지사항을 삭제하시겠습니까?')){
                location.href ="/adm/notice/delete/" + id;
            }
        }

        function fn_add_notice(){
            location.href="/adm/notice/add-form";
        }
    ...
    <div class="wrap">
        <div class="table">
            ...
            <div class="search">
                ...
                <span class="add_notice" onClick="fn_add_notice()">작성</span>
            </div>
                ...
                <c:when test="${not empty list}">
                    <c:forEach var="article" items="${list}">
                        <tr class="content" onClick="detail_board(${article.boardId})">
                            ...
                            <td><span onClick="event.stopPropagation(); fn_delete(${article.boardId})">삭제</span></td>
                        </tr>
                    </c:forEach>
                </c:when>
                <c:otherwise>
            ...
            <div class="page">
                ...
                <a class="s_f" href="/adm/notice/list?pageNum=0&keyword=${keyword}">
                    <span class="material-icons-outlined sf">first_page</span>
                </a>
                <c:if test="${pageNum > 0}">
                    <a class="pageNum" href="/adm/notice/list?pageNum=${pageNum-1}&keyword=${keyword}">
                        ...
                        <c:when test="${keyword == null}">
                            <c:choose>
                                <c:when test="${pageNum == i}">
                                    <span class="pageNum"><a class="selected" href="/adm/notice/list?pageNum=${i}">${i + 1}</a></span>
                                </c:when>
                                <c:otherwise>
                                    <span class="pageNum"><a href="/adm/notice/list?pageNum=${i}">${i + 1}</a></span>
                                </c:otherwise>
                            </c:choose>
                        </c:when>
                        <c:when test="${keyword != null}">
                            <c:choose>
                                <c:when test="${pageNum == i}">
                                    <span class="pageNum"><a class="selected" href="/adm/notice/list?pageNum=${i}&keyword=${keyword}">${i + 1}</a></span>
                                </c:when>
                                <c:otherwise>
                                    <span class="pageNum"><a href="/adm/notice/listpageNum=${i}&keyword=${keyword}">${i + 1}</a></span>
                                </c:otherwise>
                            </c:choose>
                        </c:when>
                    </c:choose>
                ...
                <a class="s_f" href="/adm/notice/list?pageNum=${pageCnt}&keyword=${keyword}">
